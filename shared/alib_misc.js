...
get_fn = function(str) {
  var fn_path = str.split("/").slice(0, -1).join("/");
  if (fn_path == "") fn_path = str.split("\\").slice(0, -1).join("\\"); //split to array & remove last element
  if (fn_path == "") fn_path = ".\\";

  if (jscript) {
    return {
      //                                                      example: c:\temp\naimi.txt
      f: fs.GetAbsolutePathName(str), //                               c:\temp\naimi.txt
      // fn  : fs.GetFile(str), //                                     DNW
      l3ch: fs.GetBaseName(str).slice(-3), //                          imi
      n: fs.GetBaseName(str), //                                       naimi
      nx: fs.GetFileName(str), //                                      naimi.txt
      p: fn_path, //                                                   c:\temp
      // folder : fs.GetFolder(str), //                                DNW
      pr: fs.GetParentFolderName(str), //                              c:\temp
      pn: fs.GetParentFolderName(str) + "\\" + fs.GetBaseName(str), // c:\temp\naimi
      x: fs.GetExtensionName(str) //                                   txt
    };
  } else {
    return {
      n: (str.split('\\').pop().split('/').pop().split('.'))[0]
    }
  }
}
